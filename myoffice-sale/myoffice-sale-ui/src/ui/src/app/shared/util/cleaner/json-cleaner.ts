...
export class JsonCleaner {
  ...
  static clean(obj: any): any {
    if (_.isArray(obj)) { // rebuild array recursively by inserting only not empty items
      ...
      return cleanArray;
    } else if (_.isObject(obj)) { // rebuild object properties recursively by inserting only not empty properties
  ...
  private static emptyValue(property: null | undefined | object): boolean {
    return _.isNull(property) || _.isUndefined(property) || ((_.isArray(property) || _.isString(property)) && _.isEmpty(property));
  }
}

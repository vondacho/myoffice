...
import static edu.noia.myoffice.invoicing.rest.processor.Processors.forDebt;
...
public class DebtEventEndpoint {
    ...
    public Flux<InvoicingEvent> getEventFlux(@PathVariable("id") DebtId debtId) {
        return Flux.create(sink -> InvoicingEventFluxSinkRegistrar.register(sink,
                event -> forDebt(debtId).apply(event), invoicingEventBroker));
    }
